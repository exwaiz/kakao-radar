...
작성일: 2026-09-16 · 갱신: 2026-09-17 · 상태: Android 0.3.0 / 서버 0.4.0 M3 선행 구현, M1 실측·외부 AI·사용자 품질 평가 대기

## M3 구현 상태

M2 커밋 `672092c`에서 관심 프로필과 주제·근거 요약을 추가한다. 영속 작업의 대상 ID·프로필/프롬프트 버전, 소유권 fencing, 방별 중복 억제, 일일 사용량 예약·정산, 실패/보류 상태, 원문 만료 표시와 표본 평가를 구현한다. 요약 저장과 작업 완료는 같은 트랜잭션이다. 외부 API 호출이 없는 추출형 기준선을 제공하고 실제 LLM 어댑터는 자격 증명·공급자 선택 후 연결한다. M4 발송 대기함/채널은 목표 설계로 남는다. 모듈과 실행 계약은 [M3_ANALYSIS.md](M3_ANALYSIS.md)를 따른다.

Android 구현은 알림 수집기·Room 저장·구조 진단·JSONL 내보내기와 M2 WorkManager 업로드다. 2026-09-17 v0.2.0에서 conversationTitle 누락 대응과 보수적 다단계 parser를 구현했고 기존 실제 방 알림 해석을 확인했다. v0.3.0은 DB 1→2→3 migration과 FastAPI/PostgreSQL 합성 HTTPS 연결을 검증했다. M1의 남은 작업은 **대상 방 선택 후 신규 저장·중복 억제·연속 알림/재부팅 시 room identity·야간 지속성 검증**이다. 사용자는 M1 실측 대기 중 M2/M3 선행 개발을 승인했다. 서버 0.4.0의 M3 분석은 위 구현 상태를 따른다. 외부 LLM 어댑터와 본폰 알림 전달은 대기다. [M2 실행과 계약](M2_SYNC.md), [README](../README.md), [단말 검증](DEVICE_TEST.md), [최신 관찰](DEBUG_NOTES.md)을 따른다.
...
M1은 수집과 로컬 DB, M2는 동기화와 서버 저장, M3는 관심·근거 요약, M4는 알림 채널을 담당한다. M3까지 서버 처리 코드를 구현했고 추출형 기준선으로 합성 검증했다. 실제 운영 서버는 배포하지 않았다. 위 그림의 외부 LLM과 발송 경로는 후속 연결 목표다.
...
Worker 중단 시 lease 만료 후 재개한다. M3의 요약 저장과 작업 완료는 같은 트랜잭션이며 부분 저장 오류는 전체 롤백한다. M4는 이 커밋에 발송 대기 생성을 더할 목표다. 공급자 호출은 DB 트랜잭션 밖에서 수행하므로 중단 후 재호출 비용이 발생할 수 있다. 시도 횟수와 예산 예약에 반영한다.
