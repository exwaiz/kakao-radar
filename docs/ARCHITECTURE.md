...
Worker 중단 시 lease 만료 후 재개한다. 요약 저장·처리 완료 표기·발송 대기 생성은 일관된 트랜잭션으로 처리한다. 외부 LLM 호출은 DB 트랜잭션 밖에서 수행하므로 중단 후 재호출 비용이 발생할 수 있다. 시도 횟수와 예산에 반영한다.
...
구체적인 통과 기준과 후속 순서는 [PROJECT_BRIEF.md의 마일스톤](PROJECT_BRIEF.md#6-마일스톤과-판단-지점)을 따른다. 현재 문서는 설계와 공식 API 제약을 검토한 결과이며 실기기 검증 결과를 포함하지 않는다.
