...
사용자는 폰을 조작할 수 없는 동안 다음 마일스톤 개발을 먼저 진행하도록 승인했다. 이 문서의 합성 데이터 결과는 M1 실제 수집 성공이나 수집률을 의미하지 않는다. M2 이후 추가한 M3 선행 구현은 [M3_ANALYSIS.md](M3_ANALYSIS.md)를 따른다. 외부 LLM과 M4 본폰 전달은 대기다.
